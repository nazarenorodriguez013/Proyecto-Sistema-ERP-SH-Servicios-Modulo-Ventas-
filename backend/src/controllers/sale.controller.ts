...
export const create = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const { items, medioPago, montoRecibido, clienteId } = req.body;
    if (!items || !items.length) {
      res.status(400).json({ message: 'El comprobante no tiene productos' });
      return;
    }
    const venta = await saleService.createSale(usuarioId, items, medioPago ?? 'Efectivo', montoRecibido ?? null, clienteId ?? null);
